...
class NaverShoppingCrawler:
    ...
    def _parse_product_card(self, card: Dict, collection_timestamp: Optional[str] = None) -> Optional[Dict]:
        ...
        # 네이버 쇼핑 상품 URL 형식에 맞춰 구성
        product_url = f"https://shopping.naver.com/window-products/beauty/{product_id}" if product_id else "N/A"
        ...
        parsed_data = {
            ...
            "keepCount": product.get('keepCount', 0) # 찜하기 개수
        }
...
def main_collect_products_by_categories():
    ...
    # 실제 카테고리 ID는 네이버 쇼핑 해당 카테고리 페이지 URL에서 'displayCategoryId=' 뒤의 값을 확인하세요.
    target_category_ids = {
        "뷰티_전체": "20006491",
        "스킨케어": "20006492",
        "선케어": "20006505",
    ...
    # 수집 목표: 각 카테고리별로 인기순 1000개 데이터
    items_per_category = 1000
    page_size = 20 # 한 페이지에 20개 상품씩 가져옴 (API 기본)
    max_pages_to_fetch = (items_per_category + page_size - 1) // page_size # 1000개 / 20개 = 50페이지
    ...
    for category_name, category_id in target_category_ids.items():
        try:
            ...
            # 수집된 상품 개수 확인 (정확히 1000개가 아닐 수 있음, API 응답에 따라 달라짐)
            actual_items_collected = len(products_for_category)
            logger.info(f"✅ 카테고리 '{category_name}' 크롤링 완료! 총 {actual_items_collected}개 상품 수집.")
            
            # --- 파일 이름 구성 변경 ---
            csv_file = f"{data_dir}/{current_timestamp}_{category_name}_{category_id}.csv"
            json_file = f"{data_dir}/{current_timestamp}_{category_name}_{category_id}.json"
            # --- 파일 이름 구성 변경 끝 ---
            ...
            for i, product in enumerate(products_for_category[:1], 1):
                ...
                print(f"   URL: {product.get('productUrl', 'N/A')}")
